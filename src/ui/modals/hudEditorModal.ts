...
function createSubtitleSlotOverlays(): HudOverlay[] {
    const positions: Array<Pick<HudOverlay, "anchor" | "x" | "y">> = [
...
function buildTemplateOverlays(template: HudTemplateName): HudOverlay[] {
    if (template === "subtitle_slots_5") return createSubtitleSlotOverlays();
...
function buildHudEditor(): void {
    ...
    form.querySelectorAll<HTMLButtonElement>(".hudTemplateBtn").forEach((button) => {
        button.onclick = () => {
            const overlays = buildTemplateOverlays(button.dataset.template as HudTemplateName);
...
function subtitleVisibleBindingName(index: number): string {
    return `#visible${index + 1}`;
}

function buildSubtitleDataPanel(overlays: HudOverlay[]): Record<string, any> {
