...
type HudSourceType = HudOverlayId;
...
type HudOverlay = {
    id: HudOverlayId; label: string; sourceType: HudSourceType; bindingKey: string; parseMode: HudParseMode; outputType: HudOutputType;
    triggerText: string; sampleText: string; preserveValue: boolean; stripTriggerText: boolean; sliceStart: number; sliceEnd: number;
    splitDelimiter: string; splitPart: HudSplitPart; splitPrefix: string; maxValue: number; x: number; y: number; width: number; height: number;
    layer: number; visible: boolean; background: boolean; color: string;
};

const PREVIEW_WIDTH = 1500;
const PREVIEW_HEIGHT = Math.round(1500 / 1.7777777);
const BINDING_KEYS: Record<HudSourceType, string> = { title: "#hud_title_text_string", subtitle: "#hud_subtitle_text_string", actionbar: "#hud_actionbar_text_string" };
...
const state = { overlays: defaults(), selectedId: "title" as HudOverlayId, draggingId: null as HudOverlayId | null, dragOffsetX: 0, dragOffsetY: 0 };
...
function parseDescription(mode: HudParseMode): string {
    if (mode === "trigger") return "키워드가 포함될 때만 보이거나 값을 갱신합니다. 예: info:, t1:, hp_text:";
    if (mode === "slice") return "고정 길이 슬롯에서 문자열 구간을 잘라 씁니다. 예: 400~600 = hp_text, 600~800 = hp_clip";
...
function buildHudEditor(): void {
    ...
    (form.querySelector(".hudEditorResetBtn") as HTMLButtonElement).onclick = () => { state.overlays = defaults(); state.selectedId = "title"; renderHudEditor(); new Notification("HUD 배치를 초기화했습니다.", 2200, "notif"); };
    form.querySelectorAll<HTMLButtonElement>(".hudTemplateBtn").forEach((button) => button.onclick = () => {
        Object.assign(selected(), TEMPLATE_PATCHES[button.dataset.template as HudTemplateName]); selected().bindingKey = BINDING_KEYS[selected().sourceType]; renderHudEditor(); new Notification(`${button.textContent} 템플릿을 적용했습니다.`, 2200, "notif");
    });
    (form.querySelector(".hudEditorCopyBtn") as HTMLButtonElement).onclick = async () => {
        try { await navigator.clipboard.writeText(generateHudJson()); new Notification("HUD JSON을 복사했습니다.", 2200, "notif"); }
        catch (error) { console.error(error); new Notification("HUD JSON을 복사하지 못했습니다.", 2800, "error"); }
    };
    (form.querySelector(".hudEditorDownloadBtn") as HTMLButtonElement).onclick = () => {
        const blob = new Blob([generateHudJson()], { type: "application/json" }); const url = URL.createObjectURL(blob); const link = document.createElement("a");
        link.href = url; link.download = "hud_text_editor.json"; link.click(); URL.revokeObjectURL(url);
    };
}
...
function renderOverlayList(): void {
    const container = getForm().querySelector(".hudEditorOverlayList") as HTMLDivElement | null; if (!container) return;
    ...
    container.querySelectorAll<HTMLButtonElement>(".hudEditorOverlayListItem").forEach((button) => button.onclick = () => { state.selectedId = button.dataset.overlayId as HudOverlayId; renderHudEditor(); });
}

function renderPreview(): void {
    const preview = getForm().querySelector("#hudEditorPreview") as HTMLDivElement | null; if (!preview) return;
    ...
    preview.querySelectorAll<HTMLDivElement>(".hudEditorOverlay").forEach((element) => element.onmousedown = (event) => {
        const overlay = state.overlays.find((entry) => entry.id === element.dataset.overlayId as HudOverlayId); if (!overlay) return;
        state.selectedId = overlay.id; state.draggingId = overlay.id; state.dragOffsetX = event.offsetX; state.dragOffsetY = event.offsetY; renderHudEditor(); event.preventDefault();
    });
    preview.onmousemove = (event) => {
        if (!state.draggingId) return; const previewRect = preview.getBoundingClientRect(); const overlay = state.overlays.find((entry) => entry.id === state.draggingId); if (!overlay) return;
        ...
        overlay.x = Math.max(0, Math.min(PREVIEW_WIDTH - overlay.width, Math.round(nextX))); overlay.y = Math.max(0, Math.min(PREVIEW_HEIGHT - overlay.height, Math.round(nextY)));
...
function renderInspector(): void {
    ...
    container.querySelectorAll<HTMLInputElement | HTMLSelectElement>("[data-field]").forEach((input) => {
        ...
        const handler = () => {
            ...
            if (field === "sourceType") { current.sourceType = input.value as HudSourceType; current.bindingKey = BINDING_KEYS[current.sourceType]; }
            else if (field === "parseMode") current.parseMode = input.value as HudParseMode;
            else if (field === "outputType") { current.outputType = input.value as HudOutputType; if (current.outputType === "progress_bar") current.background = true; }
            else if (field === "splitPart") current.splitPart = input.value as HudSplitPart;
            else if (input instanceof HTMLInputElement && input.type === "checkbox") (current as any)[field] = input.checked;
            else if (input instanceof HTMLInputElement && input.type === "number") (current as any)[field] = Math.max(0, Number(input.value) || 0);
            else (current as any)[field] = input.value;
            renderHudEditor();
        };
...
const colorHexToRgb = (color: string): [number, number, number] => {
    const normalized = color.replace("#", ""); const bigint = Number.parseInt(normalized, 16);
    return [((bigint >> 16) & 255) / 255, ((bigint >> 8) & 255) / 255, (bigint & 255) / 255];
};
const triggerVisibility = (bindingKey: string, triggerText: string) => !triggerText.trim() ? `(not (${bindingKey} = ''))` : `(not ((${bindingKey} - '${escapeBindingText(triggerText)}') = ${bindingKey}))`;
const splitLengthExpr = () => "(#length + 1 * (not (('%.'+#length+'s') * #string = #string)))";
const splitSearchExpr = (delimiter: string) => `(#search + 1 * (#search < #length and (('%.'+#search+'s') * #string = (('%.'+#search+'s') * #string - '${escapeBindingText(delimiter)}'))))`;
function sliceExpr(sourceValue: string, start: number, end: number): string {
    const safeStart = Math.max(0, start); const safeEnd = Math.max(safeStart, end);
    return safeStart === 0 ? `('%.${safeEnd}s' * ${sourceValue})` : `(('%.${safeEnd}s' * ${sourceValue}) - ('%.${safeStart}s' * ${sourceValue}))`;
}
function splitValueExpr(overlay: HudOverlay, sourceValue: string): string {
    const delimiter = escapeBindingText(overlay.splitDelimiter); const prefix = overlay.stripTriggerText ? escapeBindingText(overlay.splitPrefix) : "";
    if (overlay.splitPart === "first") { let expression = `(('%.'+#search+'s') * ${sourceValue})`; if (prefix) expression = `(${expression} - '${prefix}')`; if (delimiter) expression = `(${expression} - '${delimiter}')`; return expression; }
    let expression = `(${sourceValue} - (('%.'+#search+'s') * ${sourceValue}))`; if (delimiter) expression = `(${expression} - '${delimiter}')`; if (prefix) expression = `(${expression} - '${prefix}')`; return expression;
}
function dataStringExpr(overlay: HudOverlay): string {
    const sourceValue = overlay.preserveValue ? "#preserved_text" : "#string";
    if (overlay.parseMode === "slice") { let expression = sliceExpr(sourceValue, overlay.sliceStart, overlay.sliceEnd); if (overlay.stripTriggerText && overlay.triggerText.trim()) expression = `(${expression} - '${escapeBindingText(overlay.triggerText)}')`; return expression; }
    if (overlay.parseMode === "split_pair") return splitValueExpr(overlay, sourceValue);
    return overlay.stripTriggerText && overlay.triggerText.trim() ? `(${sourceValue} - '${escapeBindingText(overlay.triggerText)}')` : sourceValue;
}
function parserBindings(overlay: HudOverlay): Record<string, any>[] {
    const bindings: Record<string, any>[] = []; const visibleExpression = overlay.parseMode === "trigger" ? triggerVisibility(overlay.bindingKey, overlay.triggerText) : `(not (${overlay.bindingKey} = ''))`;
    ...
    if (overlay.parseMode === "split_pair") {
        if (overlay.preserveValue) bindings.push({ binding_type: "view", source_property_name: "#preserved_text", target_property_name: "#string" });
...
const textBindings = (overlay: HudOverlay, dataControlName: string) => [{ binding_type: "view", source_control_name: dataControlName, source_property_name: dataStringExpr(overlay), target_property_name: "#text" }, { binding_type: "view", source_control_name: dataControlName, source_property_name: "#visible", target_property_name: "#visible" }];
const progressBindings = (overlay: HudOverlay, dataControlName: string) => [{ binding_type: "view", source_control_name: dataControlName, source_property_name: dataStringExpr(overlay), target_property_name: "#raw_value" }, { binding_type: "view", source_property_name: "(#raw_value * 1)", target_property_name: "#health" }, { binding_type: "view", source_property_name: `(((${overlay.maxValue} - #health) / ${overlay.maxValue}))`, target_property_name: "#clip_ratio" }, { binding_type: "view", source_control_name: dataControlName, source_property_name: "#visible", target_property_name: "#visible" }];

function overlayControl(overlay: HudOverlay): Record<string, any> {
    const [r, g, b] = colorHexToRgb(overlay.color); const controlName = `${overlay.id}_control`; const dataControlName = `${overlay.id}_data_control`;
...
function renderHudEditor(): void { renderOverlayList(); renderPreview(); renderInspector(); }
function closeHudEditor(): void { getModal().style.display = "none"; state.draggingId = null; }
export async function hudEditorModal(): Promise<void> { buildHudEditor(); renderHudEditor(); getModal().style.display = "block"; getCloseButton().onclick = () => closeHudEditor(); }
