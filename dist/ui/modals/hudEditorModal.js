...
function createDefaultProgressBar(id, label) {
    return {
        id,
        label,
        enabled: true,
...
function getSelectedElement() {
    return state.elements[state.selectedId] ?? state.progressBars.find((bar) => bar.id === state.selectedId) ?? state.elements.title;
}
function isProgressBarElement(element) {
...
function getProgressBarById(id) {
    return state.progressBars.find((bar) => bar.id === id);
}
function getBindingNameForChannel(channel) {
    switch (channel) {
        case "title":
...
function getProgressBarDataControlName(bar) {
    return `progress_bar_data_${bar.id.replace(/[^a-zA-Z0-9_]/g, "_")}`;
}
function getProgressBarInsertName(bar) {
    return `progress_bar_${bar.id.replace(/[^a-zA-Z0-9_]/g, "_")}`;
}
function getProgressBarPrefixMatchExpression(bar, sourceName) {
    return bar.prefix
...
function buildAnimatedProgressBarTemplate() {
    return {
...
function buildProgressBarData(bar) {
...
function buildProgressBarInstance(bar) {
...
function mergeVanillaHidePatch(existing, channel, prefix) {
...
function hasEnabledProgressBars() {
    return state.progressBars.some((bar) => bar.enabled);
}
function updateOutput() {
    const hudOutput = getForm().querySelector(".hudEditorHudOutput");
    if (hudOutput)
        hudOutput.value = buildHudJson();
    const animatedOutput = getForm().querySelector(".hudEditorAnimatedBarOutput");
    if (animatedOutput)
        animatedOutput.value = buildAnimatedBarJson();
    const defsOutput = getForm().querySelector(".hudEditorUiDefsOutput");
    if (defsOutput)
        defsOutput.value = buildUiDefsJson();
    const progressCards = getForm().querySelectorAll(".hudEditorProgressSplitCard");
...
function previewProgressBarValues(bar) {
...
function buildProgressBarScriptHelper(bar) {
...
function renderSidebar() {
    ...
    container.querySelector(".hudEditorAddProgressBar")?.addEventListener("click", () => {
...
function renderCanvas() {
    ...
    preview.querySelectorAll(".hudEditorPreviewItem").forEach((item) => {
        item.addEventListener("mousedown", (event) => {
            ...
            const element = state.elements[id] ?? getProgressBarById(id);
            if (!element)
                return;
            const rawSlotIndex = item.dataset.slotIndex;
            const slotIndex = rawSlotIndex != null ? Number.parseInt(rawSlotIndex, 10) : undefined;
            if (!isProgressBarElement(element) && isSliceMode(element) && !Number.isFinite(slotIndex)) {
                return;
            }
            state.selectedId = id;
            state.drag = {
...
function renderInspector() {
    ...
    if (isProgressBarElement(element)) {
        ...
        inspector.querySelector(".hudEditorDeleteProgressBar")?.addEventListener("click", () => {
...
function renderScriptHelper() {
    ...
    container.querySelectorAll(".hudEditorCopyProgressScript").forEach((button) => {
...
function bindStaticActions() {
    ...
    const copyAnimatedButton = form.querySelector(".hudEditorCopyAnimatedBarJson");
    const downloadAnimatedButton = form.querySelector(".hudEditorDownloadAnimatedBarJson");
    const copyUiDefsButton = form.querySelector(".hudEditorCopyUiDefsJson");
    const downloadUiDefsButton = form.querySelector(".hudEditorDownloadUiDefsJson");
...
function attachDragHandlers() {
    ...
    const finishDrag = () => {
        if (!state.drag)
            return;
        const element = state.elements[state.drag.id] ?? getProgressBarById(state.drag.id);
        if (!element)
            return;
