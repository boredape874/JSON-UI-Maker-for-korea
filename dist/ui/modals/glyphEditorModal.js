...
function ensureWorkingCanvas(image) {
...
function redrawGlyphCanvas() {
    if (!state.displayCanvas || !state.displayContext)
        return;
...
async function loadBuiltInSheet(sheetName) {
    try {
...
async function handleInsertImageUpload(file) {
    try {
...
function buildGlyphEditor() {
    ...
    `;
    const sheetSelect = form.querySelector(".glyphEditorSheetSelect");
    const loadBuiltInButton = form.querySelector(".glyphEditorLoadBuiltIn");
    const findEmptyButton = form.querySelector(".glyphEditorFindEmpty");
    const uploadSheetButton = form.querySelector(".glyphEditorUploadSheetButton");
    const uploadSheetInput = form.querySelector(".glyphEditorUploadSheetInput");
    ...
    if (state.displayContext)
        state.displayContext.imageSmoothingEnabled = false;
...
//# sourceMappingURL=glyphEditorModal.js.map